...
#' @export
mordm.colorize <- function(set, objectives, mark=NULL, palette=NULL, n=100, offset=0, colors=NULL, clim=NULL, unmarked="#888888FF", alpha=1, crev=TRUE) {
	if (is.null(palette)) {
		palette <- getOption("mordm.palette", heat.colors)
	}
...
mordm.evaluate.uncertainties <- function(samples, satisficing.fcn=NULL, factors=NULL, baseline_factors=NULL, custom.fcn=NULL) {
	...
	regret.type2 <- function(original.point, samples.current) {
		maximize <- models[[1]]$maximize
		nvars <- models[[1]]$nvars
		nobjs <- models[[1]]$nobjs
		names <- models[[1]]$names
		nfactors <- nrow(samples[[1]]$objs)
		
		max(sapply(1:nobjs, function(j) {
			quantile(sapply(1:nfactors, function(i) {
				if ((nvars+j) %in% maximize || names[nvars+j] %in% maximize) {
					best <- max(sapply(1:length(samples), function(k) {
						samples[[k]]$objs[i,j]
					}))
				} else {
					best <- min(sapply(1:length(samples), function(k) {
						samples[[k]]$objs[i,j]
					}))
				}
				
				abs((samples.current$objs[i,j] - best) / samples.current$objs[i,j])
			}), probs=0.9)
		}))
	}
	
	# The following regret metrics use averages and are more suseptible to outliers
# 	regret.type1 <- function(original.point, samples) {
# 		mean(sapply(1:nrow(samples$objs), function(i) {
# 			norm((samples$objs[i,] - original.point[(nvars+1):(nvars+nobjs)]) / original.point[(nvars+1):(nvars+nobjs)], "2")# + constraint.penalty*sum(samples$constrs[i,])
# 		}))
# 	}
# 	
# 	regret.type2 <- function(original.point, samples.current) {
...
# 		nfactors <- nrow(samples[[1]]$objs)
# 		
# 		mean(sapply(1:nfactors, function(i) {
# 			best <- sapply(1:nobjs, function(j) {
# 				if ((nvars+j) %in% maximize || names[nvars+j] %in% maximize) {
# 					max(sapply(1:length(samples), function(k) {
# 						samples[[k]]$objs[i,j]
# 					}))
# 				} else {
# 					min(sapply(1:length(samples), function(k) {
# 						samples[[k]]$objs[i,j]
# 					}))
# 				}
# 			})
# 			
# 			norm(samples.current$objs[i,] - best, "2")
# 		}))
# 	}
	...
	if (!is.null(satisficing.fcn)) {
		...
		result.satisficing.type1 <- 1 - result.satisficing.type1
		...
		if (!is.null(factors)) {
			...
			result.satisficing.type2 <- -result.satisficing.type2
		}
	}
	...
	if (!is.null(satisficing.fcn)) {
		if (!is.null(factors) && !is.null(baseline_factors)) {
			...
			result <- cbind(result.regret.type1, result.regret.type2, result.satisficing.type1)
			colnames(result) <- c("Regret Type I", "Regret Type II", "Satisficing Type I")
			
		}
	} else {
		result <- cbind(result.regret.type1, result.regret.type2)
		colnames(result) <- c("Regret Type I", "Regret Type II")
	}

	if (!is.null(custom.fcn)) {
		result.custom <- sapply(1:length(samples), function(i) {
			custom.fcn(set.orig[i,], samples[[i]])
		})
		
		colnames.old <- colnames(result)
		result <- cbind(result, result.custom)
		colnames(result) <- c(colnames.old, "Custom")
	}
...
mordm.weight <- function(data, weights) {
	...
	weights <- sapply(1:nobjs, function(i) {
		if (nvars+i %in% maximize || names[nvars+i] %in% maximize) {
			weights[i]	
		} else {
			-weights[i]
		}
