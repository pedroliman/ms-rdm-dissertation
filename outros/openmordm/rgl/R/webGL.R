                               ...
                               
{
  ...
  scriptheader <- function() subst(
    ...
    var %prefix%WebGLClass = function(){
    this.prefix="%prefix%";
    this.zoom = 1;
  ...
  setnormMatrix <- function() {
    ...
    subst(
      ...
      normMatrix.multRight( this.userMatrix );',
  ...
  init <- function(id, type, flags) {
    ...
    if (!sprites_3d)
      result <- c(result, subst(
        ...
        gl.attachShader(prog%id%, this.getShader( gl, "%prefix%vshader%id%" ));
        gl.attachShader(prog%id%, this.getShader( gl, "%prefix%fshader%id%" ));
  ...
  scriptMiddle <- function() {
    ...
    c(subst(
      ...
      if (scene_needs_sorting) setprmvMatrix,
  ...
  scriptEnd <- subst(
    ...
    %prefix%WebGL = new %prefix%WebGLClass();
