...
void parseArgumentArray(char* argument, int size, double* values) {
	...
	for (i=0; i<size; i++) {
		...
		/* shortcut to fill in all remaining arguments */
		if ((*argument == '.') && (*(argument+1) == '.') && (*(argument+2) == '.') && (*(argument+3) == '\0')) {
			for (j=i; j<size; j++) {
				values[j] = values[i-1];
			}
...
#ifdef WIN32
inline void timersub(const struct timeval* start, const struct timeval* end, struct timeval* result) {
...
double timediff(struct timeval* start, struct timeval* end) {
	...
	return diff.tv_sec + (diff.tv_usec / (double)1000000L);
}

void printtime(double elapsedTime) {
...
void printbar(int currentEvaluations, int maxEvaluations, double elapsedTime) {
	int j;
	int progress = (int)(100 * currentEvaluations / (double)maxEvaluations);
	int length = 25;
	int digits = maxEvaluations > 0 ? floor(log10(maxEvaluations))+1 : 1;

	printf("\r[");

	for (j=0; j<length; j++) {
		if (j == progress/(100/length)) {
			printf(">");
		} else if (j < progress/(100/length)) {
			printf("=");
		} else {
			printf(" ");
		}
	...
	printf("] %3d%% E: ", progress);
	printtime(elapsedTime);
	printf(", R: ");

	if (currentEvaluations > 100) {
		printtime((long)((elapsedTime / currentEvaluations) * (maxEvaluations-currentEvaluations)));
	} else {
		printf("??:??:??");
	}

	printf(", N: %*d/%d", digits, currentEvaluations, maxEvaluations);
	printf(" ");
	fflush(stdout);
}
...
void runWithParameters() {
	...
	/* load and resume from the checkpoint file if the file exists */
	if (checkpointFile) {
		checkpoint = fopen(checkpointFile, "r");

		if (checkpoint) {
			fprintf(stderr, "Resuming from checkpoint file!\n");
	...
	if (outputFile && !samples) {
		printbar(BORG_Algorithm_get_nfe(algorithm), maxEvaluations, 0);
	}

	while ((currentEvaluations = BORG_Algorithm_get_nfe(algorithm)) < maxEvaluations) {
		BORG_Algorithm_step(algorithm);

		currentEvaluations = BORG_Algorithm_get_nfe(algorithm);
		gettimeofday(&now, NULL);

		/* print the progress bar */
		if (timediff(&start, &now)-lastProgress > 1.0) {
			if (outputFile && !samples) {
				lastProgress = timediff(&start, &now);
				printbar(currentEvaluations, maxEvaluations, lastProgress);
			}
		}

		/* periodically save runtime dynamics */
		...
		if (checkpointFile && (currentEvaluations - lastCheckpoint >= checkpointFrequency)) {
			signal_handler = signal(SIGINT, SIG_IGN);
			checkpoint = fopen(checkpointFile, "w");
			...
			if (checkpoint) {
				...
				fprintf(stderr, "Unable to save checkpoint file: %s\n", strerror(errno));
			}

			signal(SIGINT, signal_handler);
			lastCheckpoint = currentEvaluations;
		}
	}

	if (outputFile && !samples) {
		gettimeofday(&now, NULL);
		printbar(maxEvaluations, maxEvaluations, timediff(&start, &now));
		printf("\n");
	}
	...
	/* delete the checkpoint file after the result is successfully written */
...
int main(int argc, char* argv[]) {
	...
	while ((opt = getopt(argc, argv, "hn:o:v:l:u:c:e:d:C:f:R:F:k:K:p:i:s:SH:P:D:I:M:U:X:")) != -1) {
		switch (opt) {
		case 'h':
			...
			printf("   -k <file>   : Checkpoint file to resume interrupted runs\n");
			printf("   -K <value>  : Frequency (NFE) for saving checkpoint file\n");
			...
			printf("       -e 0.01,0.01 -n 100000 -f output.txt -- python dtlz2.py\n");       
			printf("\n");
	...
	if (!epsilons) {
		fprintf(stderr, "The -e option is required\n");
		exit(EXIT_FAILURE);
	}
	...
	if (checkpointFile && parameterFile) {
		fprintf(stderr, "Checkpointing only available in single run mode\n");
		exit(EXIT_FAILURE);
	}
...
#ifdef WIN32
	if (useSocket) {
		fprintf(stderr, "Sockets not supported on Windows\n");
		exit(EXIT_FAILURE);
	}
	...
	if (!CreatePipe(&outRead, &outWrite, &securityAttrs, 0)) {
		fprintf(stderr, "Unable to establish communication with executable\n");
		exit(EXIT_FAILURE);
	}
	...
	if (!CreatePipe(&inRead, &inWrite, &securityAttrs, 0)) {
		fprintf(stderr, "Unable to establish communication with executable\n");
		exit(EXIT_FAILURE);
	}
	...
	if (!success) {
		printf("Unable to start executable\n");
		exit(EXIT_FAILURE);
	}
	...
	int fdWrite = _open_osfhandle((intptr_t)inWrite, 0);
	int fdRead = _open_osfhandle((intptr_t)outRead, 0);
	...
	pipein = _fdopen(fdRead, "r");
	pipeout = _fdopen(fdWrite, "w");
	...
	// need to enclose in bracket to match else case
	if (1) {
#else
	if (!useSocket) {
		if (pipe(pipetoexe) == -1) {
