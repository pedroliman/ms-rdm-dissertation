...
Dos capítulos anteriores da dissertação eu tenho:
1 - Introdução:

- Contextualização: Argumentação sobre os desafios que a incerteza impõe à tomada de decisão estratégica, e argumentação sobre as limitações das demais abordagens de suporte à decisão sob incerteza.

- Objeto da Pesquisa: Avaliação de Decisões Estratégicas sob incerteza profunda.
...
- Objetivo de Pesquisa: "Analisar as contribuições da EMA e do RDM para a avaliação das decisões estratégicas em situações de incerteza profunda."
...
Falar sobre cada modelo e mostrar o Quadro da análise dos modelos. Considerar que cada um dos 
...
4.1 Ferramenta Computacional para a Análise RDM

A primeira barreira para a realização da Análise RDM é a disponibilidade de ferramentas computacionais para a operacionalização da análise. Embora existam frameworks de desenvolvimento potencialmente úteis para a modelagem exploratória (como o EmaWorkbench (KWAKKEL, 2013) e o OpenMORDM (HADKA et al., 2015)), tais ferramentas implicam em empecilhos para a utilização no contexto deste trabalho. 

A ferramenta EmaWorkbench, desenvolvida na linguagem python não possui interface gráfica, não suporta integração com o software de dinâmica de sistemas iThink, ou com modelos desenvolvidos na linguagem R. Neste sentido, a ferramenta requer que o modelo seja desenvolvido em uma ferramenta como o Vensim, Excel ou um modelo no próprio Python. A ferramenta OpenMORDM é focalizada em problemas multi-objetivo, o que está fora do escopo deste trabalho.

Considerando a necessidade de flexibilidade durante a execução deste trabalho, portanto, o pesquisador optou por desenvolver rotinas computacionais próprias para a analise dos dados. Ao fazer isto, será possível realizar a análise RDM sem a necessidade de recorrer à ferramentas terceiras ou privadas. Além disto, o desenvolvimento desta ferramenta computacional permitirá que os resultados desta dissertação sejam reproduzidos pelo seu leitor. Deste modo, procura-se atender aos requisitos de reprodutibilidade em trabalhos baseados em simulação computacional preconizados por Rahmandad e Sterman (2012).
