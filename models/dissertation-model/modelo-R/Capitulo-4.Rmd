...
# a complex table with merged cells, column/row separators, and custom alignments and widths
...
- Evolução Tecnológica das Impressoras 3D;
 ...
 - Número de Patentes relacionadas à impressão 3D:
 (Gráfico).
 
 - Investimento em P & D de empresas de impressoras 3D;
 
 - Investimento e P e D gera patentes, que expiram com o tempo:
 ...
 Patentes na Impressão 3D:
 Este artigo mostra os principais players da impressão 3D.
 https://www.finnegan.com/en/insights/3d-printing-patent-landscape.html
 
 História da Formlabs:
 https://techcrunch.com/2012/09/27/3d-printer-form-1-gets-6x-its-100k-funding-goal-on-kickstarter-in-one-day/
...
Um exemplo é a impressoã de materiais metálicos. A tecnologia foi criada inicialmente em 1984, 
...
```{r, fig.cap="Receita e Investimento - 3D Systems"}
#print(plot_receita_investimento_3dsystems)
```



```{r, fig.cap="Lucro Líquido - 3D Systems"}
# print(plot_cash_net_income_3dsystems)
```
...
$$D_{i}/dt = (Receita * Orc) - D_i/TempoMédio$$


## Síntese das Modificações Realizadas
...
A modelagem exploratória, per si, abandona a premissa de que modelos de simulação computacional apenas serão úteis se validados experimentalmente (Bankes XX). Ainda assim, os modelos podem ser verificados visando avaliar sua consistência interna, bem como os seus resultados podem ser comparados com dados históricos para observar a capacidade do modelo em explicar o comportamento passado. (Sterman XXXX) Considerando estas premissas, esta seção apresenta os testes e calibrações realizadas no modelo.
...
Considerando a característica estratégica da maior parte dos dados relacionados ao modelo, foi necessário recorrer à bases de informação públicamente disponíveis. Sendo assim, foi realizada uma pesquisa junto à websites que possuíram dados financeiros relacionados à industria.


```{r}
#nomes_variaveis_fundamentos = c("Código da Variável", "Nome da variável", "Descrição")
#kable(fundamentos_ddd$Variaveis, format = "latex",longtable = T, col.names = nomes_variaveis_fundamentos, caption = "Variáveis Coletadas por Empresa") %>% kable_styling(full_width = T, font_size = 9, latex_options =c("repeat_header"))  %>%
...
```{r}
#nomes_variaveis_fundamentos = c("Código da Variável", "Nome da variável", "Descrição")
# knitr::kable(fundamentos_ddd$Dados, caption = "Fundamentos Financeiros - 3D Systems")

# kable(fundamentos_ddd$Dados, caption = "Fundamentos Financeiros - 3D Systems", format = "latex", booktabs = T) %>% 
...
|----|------------------|----------|-----------| --------------- |
| $\mu$ | Número de Unidades Vendidas por Consumidor | Produto / Consumidores | aUnitsPerHousehold |
| $\rho$ | Taxa de Disconto | adimensional | aDiscountRate |
| $\rho$ | Taxa de Disconto | adimensional | aDiscountRate |

Table: Parâmetros Incertos e Unidades de Medidas
...
Table: Parâmetros e Valores de Referência
...
### Processos Dinâmicos e Hipóteses Dinâmicas

Pontos a usar

- Evolução da Demanda de Novos Produtos.

- O Processo de Difusão das Impressoras 3D.

- Proteção de Propriedade Intelectual e Expiração de Patentes.

https://techcrunch.com/2016/05/15/how-expiring-patents-are-ushering-in-the-next-generation-of-3d-printing/

https://3dprintingindustry.com/news/3d-printing-patents-overview-81253/
...
Hipóteses Dinâmicas a Testar:

- Processo de Difusão do Produto:
 - Incerteza Principal: Tamanho do Mercado e Velocidade da Difusão do produto.

- Preço:
 - Sensibilidade do Preço aos fatores que pressionam os preços do player.
 
- Níveis de Investimento em P e D e relação com a Expiração de Patentes.


- O que é melhor para uma empresa?
- Investir muito ou pouco em PeD?
- Investir em PeD protegendo sua propriedade intelectual ou fechando seu investimento protegendo-o com Patentes?
...
#### Geração inicial de Casos Plausíveis
...
```{r parametros1, fig.cap="Tabela de Parâmetros"}
#colunas = c("Variavel","Min", "Max", "Unidade")
#nomes = c("Variável", "Mínimo", "Máximo", "Unidade")
#knitr::kable(resultados_casos_plausiveis$Inputs$Parametros[colunas], digits = 3, col.names = nomes, caption = "Ranges de Parâmetros - Rodada 1")

kable(resultados_casos_plausiveis$Inputs$Parametros)
...
```{r}
kable(head(resultados_casos_plausiveis$Ensemble, 10))
```
...
```{r}
kable(head(resultados_casos_plausiveis$DadosSimulados, 20))
```
...
```{r}
print(plot_demanda_pre_calibracao)
```

#### Comparação das Simulações com Série Histórica

Exemplo de 5 Casos Comparados com o Histórico:

```{r}
kable(t(ensemble_com_erro[1:5,variaveis_exibir_ensemble]))
```
...
```{r}
kable(parametros_cenario_menor_erro)
```
...
```{r}
percentil_utilizado_como_criterio
percentil_ssr
```

- Definindo casos Considerados plausíveis (aqueles com erro quadrado médio menor do que o percentil). Exibindo casos:
...
```{r}
str(ensemble_a_simular)
```


#### Simulando o Comportamento de Estratégias nos Casos

- Observando Combinações de Estratégias a Simular:

```{r}
kable(t(results$Inputs$LeversFull))
```
...
```{r}
str(results$DadosSimulados)
```

- Dados Selecionados do Último Período:

```{r}
summary(results$DadosUltimoPeriodo)
```
...
#### Comparação das Estratégias - Análise da Perda de Oportunidade

- As 54 Estratégias definidas foram simuladas em 200 casos, cada uma, totalizando 10800 simulações. 

- Tabela de Análise da Perda de Oportunidade

```{r}
kable(results$AnaliseRegret$ResumoEstrategias)
```
...
```{r}
plots_results$plots_whisker$plot_whisker_lever_perc_regret
```

- Qual é a Estratégia mais Robusta?

```{r}
kable(t(results$Inputs$Levers[which(results$Inputs$Levers$Lever == results$EstrategiaCandidata$Lever),]))
```

### Análise de Vulnerabilidades da Estratégia Candidata

- Dada que a estratégia mais robusta dentre as opções disponíveis foi identificada, em que circunstância esta estratégia tende a falhar?

- Primeiro, é necessário definir um critério sobre o que é "falhar". Usamos o mesmo critério utilizado para a seleção desta estratégia (Percentil 75% do Regret Percentual).

Ou seja, quando o regret percentual é maior do que:
```{r}
threshold_analise_vulnerabilidade
```

Visualizando este critério:
...
A próxima pergunta a fazer é: "Em que circunstâncias esta estratégia leva a um arrependimento percentual maior do que o critério definido"? Ou seja, em que circunstâncias a empresa pode se arrepender desta estratégia?

#### Random Forest - Feature Selection

Aplica-se aqui a técnica Random Forest para prever a variável de interesse (estratégia fracassa ou não) em função das variáveis incertas. Utiliza-se a importância da variável definida pela redução média da acuracidade de predição.

- Tabela de Seleção de Variáveis:
```{r}
kable(print(tabela_random_forest,row.names = F))
```
...
Este gráfico mostra aproximadamente a chance do caso ser de interese à medida que o market share desejado do player 2 aumenta:
...
E este gráfico mostra duas dimensões combinadas, usando o random forest:

```{r}
plot_parcial2d
```

É possível observar que os gráficos fazem sentido com os dados simulados. Observando a relação entre estas duas variáveis identificadas com o random forest com um gráfico simples de dispersão:

```{r}
dispersao_variaveis_random_forest
```

Este comportamento também pode ser observado quando observa-se a variável de perda de oportunidade diretamente.

```{r}
landscape_estrategia_comparacao
```




#### Algoritmo Boruta - Feature Selection

```{r}
plot(boruta_output, cex.axis=.5, las=3, xlab="", main="Importância das Variáveis")
```
...
Uma possível abordagem é calcular a diferença entre as médias das variáveis incertas nos casos onde há alto regret versus os casos onde há baixo regret, para cada variável incerta.

Quanto maior for a diferença entre estas médias, mais é possível que haja uma relação entre a variável incerta e o fracasso da estratégia definida.

```{r}
kable(ranking_variaveis_por_media)
```
...
## Código Fonte da Ferramenta Computacional
...
## Algoritmo para a Replicação dos Resultados
```{r}
# parse("analise.R")
```
